/* Maintains a parts database (linked list version) Programming_projects Ch17_Q3 delete*/
...
void erase(void)
{
    int number, change; struct part *p; char ch; void *temp;
    
    printf("Enter part number: ");
    scanf("%d", &number);
    if (p = find_part(number)) {
        printf("Part #%d    Name: %s which has %d on hand will be erased. Are you sure? (Y/N)",
            p->number, p->name, p->on_hand);
        scanf(" %c", &ch);
        switch (ch)
        {
