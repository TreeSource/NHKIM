...
const struct dialing_code country_codes[] = {
    ...
    {"Ukraine", 380}, {"United Kingdome", 44}, {"United States", 1}, {"Vietnam", 84}
};
...
int main(void)
{
    ...
    for (int i = 0; i < 32; ++i)
    {
        if (country_codes[i].code == dcin)
        {
            #define WORKED
            printf("The country of entered dialing code is: %s\n", country_codes[i].country);
        }
    }
    #ifndef WORKED
    printf("Error, the entered dialing code is not in the list\n");
    #endif
    return 0;
}